@extends('layouts.appAdmin')

@section('content')

<div class="right_col" role="main">
  <div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        <div class="x_title">
          <h2> Pegawai <small>SMK Daarul Fataa</small></h2>
          <ul class="nav navbar-right panel_toolbox">
            <li>
              <a href="#tambah" data-toggle="modal">
                <button type="button" class="btn btn-info btn-xs"> <i class="fa fa-plus"></i>&nbsp; Tambah</button>

                <div class="modal fade" id="tambah" aria-hidden="true" aria-labelledby="avatar-modal-label" role="dialog" tabindex="-1">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <form class="avatar-form" action="https://colorlib.com/polygon/gentelella/crop.php" enctype="multipart/form-data" method="post">
                        <div class="modal-header">
                          <button class="close" data-dismiss="modal" type="button">&times;</button>
                          <h4 class="modal-title" id="avatar-modal-label">Tambah Siswa</h4>
                        </div>
                        <div class="modal-body">

                          <form class="form-horizontal form-label-left input_mask">

                            <div class="row">
                              <div class="col-md-6">

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Nama</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="Nama Lengkap">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">NIP</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="NIP">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Bagian</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <select name="" id="" class="form-control">
                                      <option value="" disabled> Pilih Bagian </option>
                                      <option value=""> Guru </option>
                                      <option value=""> Satpam </option>
                                    </select> 
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Gelar Awal</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <select name="" id="" class="form-control">
                                      <option value="" disabled> Pilih Gelar Awal </option>
                                      <option value=""> SMK </option>
                                      <option value=""> D3 </option>
                                      <option value=""> S1 </option>
                                      <option value=""> S2 </option>
                                      <option value=""> S3 </option>
                                    </select> 
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Gelar Akhir</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <select name="" id="" class="form-control">
                                      <option value="" disabled> Pilih Gelar Akhir </option>
                                      <option value=""> SMK </option>
                                      <option value=""> D3 </option>
                                      <option value=""> S1 </option>
                                      <option value=""> S2 </option>
                                      <option value=""> S3 </option>
                                    </select> 
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Alamat</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="Alamat">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Telepon</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="Telepon">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Handphone</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="Handphone">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Email</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="email" class="form-control" placeholder="Email">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Photo</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="file">
                                  </div>
                                </div>

                              </div>
                              <div class="col-md-6">

                                <style>
                                  .col-md-9.col-sm-9.col-xs-12.margin {
                                    padding-bottom: 12px;
                                  }
                                </style>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Panggilan</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="Nama Penggilan">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Jenis Kelamin</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <select name="" id="" class="form-control">
                                      <option value="" disabled>Pilih Kelamin</option>
                                      <option value="">Laki-Laki</option>
                                      <option value="">Perempuan</option>
                                    </select>
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Tempat Lahir</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="Tempat Lahir">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Tanggal Lahir</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="date" class="form-control" placeholder="Default Input">
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Agama</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <select name="" id="" class="form-control">
                                      <option value="" disabled>Pilih Agama</option>
                                      <option value="">Islam</option>
                                      <option value="">Kristen</option>
                                      <option value="">Budha</option>
                                    </select>
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">Status</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <select name="" id="" class="form-control">
                                      <option value="" disabled>Pilih Status</option>
                                      <option value="">Belum Kawin</option>
                                      <option value="">Kawin</option>
                                    </select>
                                  </div>
                                </div>

                                <div class="form-group">
                                  <label class="control-label col-md-3 col-sm-3 col-xs-12">No Identitas</label>
                                  <div class="col-md-9 col-sm-9 col-xs-12 margin">
                                    <input type="text" class="form-control" placeholder="No Identitas">
                                  </div>
                                </div>

                              </div>

                              <div class="col-md-12" style="text-align: right;">
                                <button class="btn btn-warning"> <i class="fa fa-save"></i>&nbsp; Tambah </button>
                              </div>

                            </div>

                          </form>


                        </div>


                      </div>
                    </a>  
                  </li>
                </ul>
                <div class="clearfix"></div>
              </div>
              <div class="x_content">
                <p class="text-muted font-13 m-b-30">
                  DataTables has most features enabled by default, so all you need to do to use it with your own tables is to call the construction function: <code>$().DataTable();</code>
                </p>

                <table id="datatable" class="table table-striped table-bordered">
                  <thead>
                    <tr>
                    <th>No</th>
                      <th>NIP</th>
                      <th>Nama</th>
                      <th>Jenis Kelamin</th>
                      <th>No Telepon</th>
                      <th>Action</th>
                    </tr>
                  </thead>


                  <tbody>
                    <tr>
                      <td>Tiger Nixon</td>
                      <td>System Architect</td>
                      <td>Edinburgh</td>
                      <td>61</td>
                      <td>2011/04/25</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Garrett Winters</td>
                      <td>Accountant</td>
                      <td>Tokyo</td>
                      <td>63</td>
                      <td>2011/07/25</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Ashton Cox</td>
                      <td>Junior Technical Author</td>
                      <td>San Francisco</td>
                      <td>66</td>
                      <td>2009/01/12</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Cedric Kelly</td>
                      <td>Senior Javascript Developer</td>
                      <td>Edinburgh</td>
                      <td>22</td>
                      <td>2012/03/29</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Airi Satou</td>
                      <td>Accountant</td>
                      <td>Tokyo</td>
                      <td>33</td>
                      <td>2008/11/28</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Brielle Williamson</td>
                      <td>Integration Specialist</td>
                      <td>New York</td>
                      <td>61</td>
                      <td>2012/12/02</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Herrod Chandler</td>
                      <td>Sales Assistant</td>
                      <td>San Francisco</td>
                      <td>59</td>
                      <td>2012/08/06</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Rhona Davidson</td>
                      <td>Integration Specialist</td>
                      <td>Tokyo</td>
                      <td>55</td>
                      <td>2010/10/14</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Colleen Hurst</td>
                      <td>Javascript Developer</td>
                      <td>San Francisco</td>
                      <td>39</td>
                      <td>2009/09/15</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Sonya Frost</td>
                      <td>Software Engineer</td>
                      <td>Edinburgh</td>
                      <td>23</td>
                      <td>2008/12/13</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Jena Gaines</td>
                      <td>Office Manager</td>
                      <td>London</td>
                      <td>30</td>
                      <td>2008/12/19</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Quinn Flynn</td>
                      <td>Support Lead</td>
                      <td>Edinburgh</td>
                      <td>22</td>
                      <td>2013/03/03</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Charde Marshall</td>
                      <td>Regional Director</td>
                      <td>San Francisco</td>
                      <td>36</td>
                      <td>2008/10/16</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Haley Kennedy</td>
                      <td>Senior Marketing Designer</td>
                      <td>London</td>
                      <td>43</td>
                      <td>2012/12/18</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Tatyana Fitzpatrick</td>
                      <td>Regional Director</td>
                      <td>London</td>
                      <td>19</td>
                      <td>2010/03/17</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Michael Silva</td>
                      <td>Marketing Designer</td>
                      <td>London</td>
                      <td>66</td>
                      <td>2012/11/27</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Paul Byrd</td>
                      <td>Chief Financial Officer (CFO)</td>
                      <td>New York</td>
                      <td>64</td>
                      <td>2010/06/09</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Gloria Little</td>
                      <td>Systems Administrator</td>
                      <td>New York</td>
                      <td>59</td>
                      <td>2009/04/10</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Bradley Greer</td>
                      <td>Software Engineer</td>
                      <td>London</td>
                      <td>41</td>
                      <td>2012/10/13</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Dai Rios</td>
                      <td>Personnel Lead</td>
                      <td>Edinburgh</td>
                      <td>35</td>
                      <td>2012/09/26</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Jenette Caldwell</td>
                      <td>Development Lead</td>
                      <td>New York</td>
                      <td>30</td>
                      <td>2011/09/03</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Yuri Berry</td>
                      <td>Chief Marketing Officer (CMO)</td>
                      <td>New York</td>
                      <td>40</td>
                      <td>2009/06/25</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Caesar Vance</td>
                      <td>Pre-Sales Support</td>
                      <td>New York</td>
                      <td>21</td>
                      <td>2011/12/12</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Doris Wilder</td>
                      <td>Sales Assistant</td>
                      <td>Sidney</td>
                      <td>23</td>
                      <td>2010/09/20</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Angelica Ramos</td>
                      <td>Chief Executive Officer (CEO)</td>
                      <td>London</td>
                      <td>47</td>
                      <td>2009/10/09</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Gavin Joyce</td>
                      <td>Developer</td>
                      <td>Edinburgh</td>
                      <td>42</td>
                      <td>2010/12/22</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Jennifer Chang</td>
                      <td>Regional Director</td>
                      <td>Singapore</td>
                      <td>28</td>
                      <td>2010/11/14</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Brenden Wagner</td>
                      <td>Software Engineer</td>
                      <td>San Francisco</td>
                      <td>28</td>
                      <td>2011/06/07</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Fiona Green</td>
                      <td>Chief Operating Officer (COO)</td>
                      <td>San Francisco</td>
                      <td>48</td>
                      <td>2010/03/11</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Shou Itou</td>
                      <td>Regional Marketing</td>
                      <td>Tokyo</td>
                      <td>20</td>
                      <td>2011/08/14</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Michelle House</td>
                      <td>Integration Specialist</td>
                      <td>Sidney</td>
                      <td>37</td>
                      <td>2011/06/02</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Suki Burks</td>
                      <td>Developer</td>
                      <td>London</td>
                      <td>53</td>
                      <td>2009/10/22</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Prescott Bartlett</td>
                      <td>Technical Author</td>
                      <td>London</td>
                      <td>27</td>
                      <td>2011/05/07</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Gavin Cortez</td>
                      <td>Team Leader</td>
                      <td>San Francisco</td>
                      <td>22</td>
                      <td>2008/10/26</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Martena Mccray</td>
                      <td>Post-Sales support</td>
                      <td>Edinburgh</td>
                      <td>46</td>
                      <td>2011/03/09</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Unity Butler</td>
                      <td>Marketing Designer</td>
                      <td>San Francisco</td>
                      <td>47</td>
                      <td>2009/12/09</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Howard Hatfield</td>
                      <td>Office Manager</td>
                      <td>San Francisco</td>
                      <td>51</td>
                      <td>2008/12/16</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Hope Fuentes</td>
                      <td>Secretary</td>
                      <td>San Francisco</td>
                      <td>41</td>
                      <td>2010/02/12</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Vivian Harrell</td>
                      <td>Financial Controller</td>
                      <td>San Francisco</td>
                      <td>62</td>
                      <td>2009/02/14</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Timothy Mooney</td>
                      <td>Office Manager</td>
                      <td>London</td>
                      <td>37</td>
                      <td>2008/12/11</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Jackson Bradshaw</td>
                      <td>Director</td>
                      <td>New York</td>
                      <td>65</td>
                      <td>2008/09/26</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Olivia Liang</td>
                      <td>Support Engineer</td>
                      <td>Singapore</td>
                      <td>64</td>
                      <td>2011/02/03</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Bruno Nash</td>
                      <td>Software Engineer</td>
                      <td>London</td>
                      <td>38</td>
                      <td>2011/05/03</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Sakura Yamamoto</td>
                      <td>Support Engineer</td>
                      <td>Tokyo</td>
                      <td>37</td>
                      <td>2009/08/19</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Thor Walton</td>
                      <td>Developer</td>
                      <td>New York</td>
                      <td>61</td>
                      <td>2013/08/11</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Finn Camacho</td>
                      <td>Support Engineer</td>
                      <td>San Francisco</td>
                      <td>47</td>
                      <td>2009/07/07</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Serge Baldwin</td>
                      <td>Data Coordinator</td>
                      <td>Singapore</td>
                      <td>64</td>
                      <td>2012/04/09</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Zenaida Frank</td>
                      <td>Software Engineer</td>
                      <td>New York</td>
                      <td>63</td>
                      <td>2010/01/04</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Zorita Serrano</td>
                      <td>Software Engineer</td>
                      <td>San Francisco</td>
                      <td>56</td>
                      <td>2012/06/01</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Jennifer Acosta</td>
                      <td>Junior Javascript Developer</td>
                      <td>Edinburgh</td>
                      <td>43</td>
                      <td>2013/02/01</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Cara Stevens</td>
                      <td>Sales Assistant</td>
                      <td>New York</td>
                      <td>46</td>
                      <td>2011/12/06</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Hermione Butler</td>
                      <td>Regional Director</td>
                      <td>London</td>
                      <td>47</td>
                      <td>2011/03/21</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Lael Greer</td>
                      <td>Systems Administrator</td>
                      <td>London</td>
                      <td>21</td>
                      <td>2009/02/27</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Jonas Alexander</td>
                      <td>Developer</td>
                      <td>San Francisco</td>
                      <td>30</td>
                      <td>2010/07/14</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Shad Decker</td>
                      <td>Regional Director</td>
                      <td>Edinburgh</td>
                      <td>51</td>
                      <td>2008/11/13</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Michael Bruce</td>
                      <td>Javascript Developer</td>
                      <td>Singapore</td>
                      <td>29</td>
                      <td>2011/06/27</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      <td>Donna Snider</td>
                      <td>Customer Support</td>
                      <td>New York</td>
                      <td>27</td>
                      <td>2011/01/25</td>
                      <td>
                        <div class="btn-group" style="display: block;">
                          <a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <center><span class="caret"></span></center>    
                          <ul class="dropdown-menu" role="menu">
                          <li><a href="">Detail</a></li>
                            <li><a href="#">Edit</a>
                            </li>
                            <li><a href="#">Hapus</a>
                            </li>
                          </ul>
                        </div>
                      </td>
                    </tr>
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        </div>
      </div>

    </div>
        

@endsection